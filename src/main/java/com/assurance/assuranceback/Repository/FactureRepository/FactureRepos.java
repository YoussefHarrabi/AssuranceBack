...
public interface FactureRepos extends JpaRepository<Facture, Long> {
    ...
    @Query("SELECT new com.assurance.assuranceback.dto.SocieteStatsDTO(" +
            ...
            "SUM(CASE WHEN f.factureStatut = 'NONPAYEE' THEN f.montant ELSE 0 END), " + // montantNonPaye
            "SUM(CASE WHEN f.factureStatut = 'EnAttente' THEN f.montant ELSE 0 END), " + // montantEnAttente
            "SUM(CASE WHEN f.factureStatut = 'PAYEE' THEN 1 ELSE 0 END), " + // nombreFacturesPayees
            "SUM(CASE WHEN f.factureStatut = 'NONPAYEE' THEN 1 ELSE 0 END), " + // nombreFacturesNonPayees
